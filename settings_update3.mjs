...
const generateComponent = (funcName, titleStr, dataVar, isSales) => {
  return \`function \${funcName}({ \${dataVar}, settings, payments = [], creditNotes = [], onClose }: any) {
  const data = \${dataVar};
  ...
  const consigneeName = \${isSales ? 'data.consigneeName || data.partyName' : 'data.partyName'} || '';
  const consigneeAddress = \${isSales ? 'data.consigneeAddress || data.partyAddress' : 'data.partyAddress'} || '';
  const isInterstate = data.isInterstate || false;

  const layout = settings?.layoutSettings || { styles: { tableRowsCount: 12, paddingTop: 0, paddingBottom: 0, headerFontSize: 30 }, sectionOrder: ["religious", "header", "metadata", "table", "footer"] };
      ...
      <div key="metadata" className="flex flex-col">
        ...
        <div className="grid grid-cols-[60%_40%] border-b border-black">
          <div className="p-2 border-r border-black font-bold text-[11px] uppercase flex flex-col">
            ...
            <span className="mt-1 flex gap-2"><span>GSTIN:</span> <span>{\${isSales ? 'data.consigneeGstin || data.partyGstin' : 'data.partyGstin'} || "-"}</span></span>
          </div>
...
const printSales = generateComponent('PrintPreview', 'TAX INVOICE', 'booking', true);
const printPurchase = generateComponent('PurchasePrintPreview', 'PURCHASE VOUCHER', 'purchase', false);
const printDebit = generateComponent('DebitNotePrintPreview', 'DEBIT NOTE', 'debitNote', false);
const printCredit = generateComponent('CreditNotePrintPreview', 'CREDIT NOTE', 'creditNote', false);

const replaceFn = (fnName, newContent) => {
  const rs = new RegExp(\`function \${fnName}\\([\\s\\S]*?<\/motion\\.div>\\s*\\);\\s*\\}\`, 'm');
  return content.replace(rs, newContent);
}

content = replaceFn('PrintPreview', printSales);
content = replaceFn('PurchasePrintPreview', printPurchase);
content = replaceFn('DebitNotePrintPreview', printDebit);
content = replaceFn('CreditNotePrintPreview', printCredit);
